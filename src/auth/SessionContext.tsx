...
export function AuthProvider({ children }: AuthProviderProps) {
    ...
    try {
      ...
      if (user) {
        ...
        dispatch({
          type: Types.INITIAL,
          payload: {
            isAuthenticated: user.twoFactorAuth?.isEnabled || false,
            isOtpValid: false,
            signUpComplete: false,
            isLoading: false,
            user,
  ...
  const resetwithtoken = useCallback(
    async (password: string, cpassword: string, token: string) => {
      ...
      try {
        ...
        const response = await axiosInstance.post('/user/resetwithtoken', { password, cpassword, token });

        dispatch({ type: Types.INITIAL, payload: { ...initialPayload, isLoading: false } });
  ...
  const completeSignUp = useCallback(
    async (settings: any) => {
      try {
        ...
        const response = await axiosInstance.post(`/user/final/${state?.user?.id}`, {
          ...settings,
